...
pub fn createSligAlertMsg(sequence: u64, alert: SligAlert) Message {
    return Message{
        .header = MessageHeader{
            ...
            .timestamp = std.time.microTimestamp(),
            .msg_type = .SligAlert,
            .payload_size = @sizeOf(SligAlert) + 
                (if (alert.evidence) |e| e.len else 0) + 
                alert.category.len + alert.message.len,
        },
...
pub fn packetEventFromCaptureInfo(
    packet: *const capture.PacketInfo,
    flow_id: u64
) PacketEvent {
...
pub fn toBinary(self: *const Message, allocator: std.mem.Allocator) ![]u8 {
    ...
    // Write header
    std.mem.writeIntNative(u16, buffer[0..2], self.header.version);
    std.mem.writeIntNative(u64, buffer[2..10], self.header.sequence);
    std.mem.writeIntNative(i64, buffer[10..18], self.header.timestamp);
    buffer[18] = @intFromEnum(self.header.msg_type);
    std.mem.writeIntNative(u32, buffer[19..23], self.header.payload_size);
    ...
    switch (self.payload) {
        .Hello => |p| {
            ...
            std.mem.writeIntNative(u16, buffer[offset..offset+2], p.client_version);
            offset += 2;
            std.mem.writeIntNative(u32, buffer[offset..offset+4], p.capabilities);
            offset += 4;
            
            // Write string length and content
            std.mem.writeIntNative(u32, buffer[offset..offset+4], @intCast(p.client_name.len));
            offset += 4;
            std.mem.copy(u8, buffer[offset..], p.client_name);
        },
        .SligAlert => |p| {
            var offset: usize = @sizeOf(MessageHeader);
            
            // Write fixed-size fields first
            std.mem.writeIntNative(u64, buffer[offset..offset+8], p.alert_id);
            offset += 8;
            std.mem.writeIntNative(i64, buffer[offset..offset+8], p.timestamp);
            offset += 8;
            ...
            // Write category
            std.mem.writeIntNative(u32, buffer[offset..offset+4], @intCast(p.category.len));
            offset += 4;
            std.mem.copy(u8, buffer[offset..], p.category);
            offset += p.category.len;
            
            // Write message
            std.mem.writeIntNative(u32, buffer[offset..offset+4], @intCast(p.message.len));
            offset += 4;
            std.mem.copy(u8, buffer[offset..], p.message);
            ...
            if (p.evidence) |evidence| {
                ...
                offset += 1;
                std.mem.writeIntNative(u32, buffer[offset..offset+4], @intCast(evidence.len));
                offset += 4;
                std.mem.copy(u8, buffer[offset..], evidence);
            } else {
...
pub fn fromBinary(data: []const u8, allocator: std.mem.Allocator) !Message {
    ...
    const header = MessageHeader{
        .version = std.mem.readIntNative(u16, data[0..2]),
        .sequence = std.mem.readIntNative(u64, data[2..10]),
        .timestamp = std.mem.readIntNative(i64, data[10..18]),
        .msg_type = @enumFromInt(data[18]),
        .payload_size = std.mem.readIntNative(u32, data[19..23]),
    };
    ...
    switch (header.msg_type) {
        .Hello => {
            ...
            const client_version = std.mem.readIntNative(u16, data[offset..offset+2]);
            offset += 2;
            const capabilities = std.mem.readIntNative(u32, data[offset..offset+4]);
            offset += 4;
            
            if (offset + 4 > data.len) return error.MessageTruncated;
            const name_len = std.mem.readIntNative(u32, data[offset..offset+4]);
            offset += 4;
...
pub fn createHelloMsg(
    ...
    return Message{
        .header = MessageHeader{
            ...
            .timestamp = std.time.microTimestamp(),
            .msg_type = .Hello,
            .payload_size = @sizeOf(Message.Payload.Hello) + client_name.len,
        },
        .payload = .{ 
            .Hello = .{
                .client_version = client_version,
...
pub fn createHeartbeatMsg(sequence: u64, uptime_seconds: u64) Message {
    return Message{
        .header = MessageHeader{
            ...
            .timestamp = std.time.microTimestamp(),
            .msg_type = .Heartbeat,
            .payload_size = @sizeOf(Message.Payload.Heartbeat),
        },
        .payload = .{ 
            .Heartbeat = .{
...
pub fn createShutdownMsg(sequence: u64, reason: []const u8, restart: bool) Message {
    return Message{
        .header = MessageHeader{
            ...
            .timestamp = std.time.microTimestamp(),
            .msg_type = .Shutdown,
            .payload_size = @sizeOf(Message.Payload.Shutdown) + reason.len,
        },
        .payload = .{ 
            .Shutdown = .{
                .reason = reason,
